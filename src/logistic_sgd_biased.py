...
class LogisticRegression(object):
    ...
    def __init__(self, rng, input, n_in, n_out, W, b, bias):
...
#                                name='b', borrow=True)
    ...
    def negative_log_likelihood(self, y):
        ...
        float_y = T.cast(y,theano.config.floatX)
        decay_y=theano.tensor.set_subtensor(float_y[(float_y<1.0).nonzero()], self.class_bias)

        # log_likelihood=-T.mean(T.log(self.p_y_given_x)[T.arange(y.shape[0]), y])
        log_likelihood=T.mean(decay_y*(-T.log(self.p_y_given_x)[T.arange(y.shape[0]), y]))
