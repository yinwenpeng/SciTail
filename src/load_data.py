...
def load_sentiment_label_des(word2id):
    file = "/save/wenpeng/datasets/StanfordSentiment/stanfordSentimentTreebank/2classes/labelDescriptions.txt"
    ...
    for line in readfile:
        ...
        if parts[0]=='0':
            for word in parts[1:]:
                id = word2id.get(word)
                if id is None:
                    id=len(word2id)+1  # id of true words starts from 1, leaving 0 to "pad id"
                    word2id[word]=id
                zero_ids.append(id)
        else:
            for word in parts[1:]:
                ...
                one_ids.append(id)
    readfile.close()
    print 'label des word loaded over:', zero_ids  ,one_ids
    return   zero_ids  ,one_ids

def load_sentiment_dataset(maxlen=40, minlen=4):
    root="/save/wenpeng/datasets/StanfordSentiment/stanfordSentimentTreebank/2classes/"
